...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for(String iter : set) {
            ...
            if(iter.startsWith("R") || iter.startsWith("r")) {
                seekingCountries.add(iter);
            }
        }
    ...
    public List<String> iteratingOverEntries(){
        ...
        Set<Map.Entry<String, String>> es = countries.entrySet();
        String key;
        for(Map.Entry one : es) {
            key = (String) one.getKey();
    ...
    public String iteratingOverValues(){
        ...
        String result;
        int max = 0;
        for(String iter:countries.values()) {
            if(iter.length() > max) {
                max = iter.length();
                seekingCapital = iter;
            }
